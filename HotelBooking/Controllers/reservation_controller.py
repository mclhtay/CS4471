...
class ReservationController(Controller):
    ...
    def reserve_room(self, room_id: str, customer_id: int, startDate, duration: int):
        ...
        self.reservation.create_reservation(
            "OPEN", customer_id, room_id, bill.bill_id, startDate, duration)
    ...
    def modify_reservation_duration(self, reservation_id: int, newDuration: int):
        self.reservation.update_reservation_duration(
            reservation_id, newDuration)
        currentReservation = self.reservation.get_reservation_by_id(
            reservation_id)
        price: float = self.roomType.get_Price(
            self.room.get_room_by_id(currentReservation.room_id).room_type)
        self.bill_controller
        newBillID = self.bill_controller.modify(self.reservation.get_reservation_by_id(
            reservation_id).bill_id, price*newDuration)
